...
exports.link = {
    new: async (user, school, type, status) => {
        try {
            const result = await db.query("INSERT INTO users_schools (user_code, school_code, type, status) VALUES ( $1, $2, $3, $4 ) RETURNING *;", [ user, school, type || "owner", status || "active" ]);
            return result.rows[0];
        } catch (error) {
            return error;
        };
    ...
    one: async (user, school) => {
        try {
            const result = await db.query(`
                SELECT s.*, json_build_object('type', us.type, 'status', us.status, 'created_at', us.created_at AT TIME ZONE 'UTC') AS link 
